function QueryProposicoesComMaisTweetsPorParlamentar(interesse, dataInicial, dataFinal, id_parlaementar, qtd) {
  const q ="SELECT " +
    "tweet_p_bydate.id_proposicao_leggo, COUNT(tweet_p_bydate.id_tweet) " + 
    "FROM (" +
    "SELECT tweet_proposicao.* FROM tweet_proposicao " +
    ...
    ") AS tweet_p_bydate " +
    "GROUP BY id_proposicao_leggo " +
    "HAVING COUNT(id_tweet) > 0 " +
    "ORDER BY 2 DESC " +
    "LIMIT '" + qtd + "' ";
  ;
  return q;
