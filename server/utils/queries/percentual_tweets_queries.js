...
function QueryPercentualAtividadeAgregadaPorAgenda(interesse, dataInicial, dataFinal) {
  ...
  "COUNT(tweet.id_tweet) OVER (" +
  "PARTITION BY id_parlamentar_parlametria) AS total, " +
  "id_tweet, " +
  "created_at " +
  "FROM tweet) AS tweet "+
  "ON tweet_proposicao.id_tweet = tweet.id_tweet AND tweet.created_at BETWEEN '"+
  dataInicial +"' AND '"+ dataFinal + "' " +
  "AND tweet_proposicao.relator_proposicao = FALSE " +
  "GROUP BY agenda.slug, tweet.id_parlamentar_parlametria, tweet.total) AS res " +
  "WHERE res.slug = '" + interesse + "';";
